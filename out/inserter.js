 ...
 */
function detectIndent(lines, aroundLine) {
...
async function addElement(parsed, element) {
    ...
    const code = (0, generator_1.generateCode)(element, parsed.className, headerIndent, sourceIndent);
    ...
    if (element.kind === 'Port') {
        const portFiles = (0, generator_1.generatePortClassFiles)(element.codeName);
        ...
        for (let i = 0; i < portLines.length; i++) {
            if (portLines[i].trimStart().startsWith('#include')) {
                portLastInclude = i;
            }
        }
    ...
    // Add to XML template
    if (parsed.xmlPath) {
        await (0, xmlEditor_1.addXmlElement)(parsed.xmlPath, element);
    }
 ...
 */
async function removeElement(parsed, element) {
    const headerDoc = await vscode.workspace.openTextDocument(parsed.headerPath);
    const sourceDoc = await vscode.workspace.openTextDocument(parsed.sourcePath);
    const headerText = headerDoc.getText();
    ...
    if (parsed.xmlPath) {
        await (0, xmlEditor_1.removeXmlElement)(parsed.xmlPath, element);
    }
 ...
 */
async function changeElement(parsed, oldElement, newElement) {
    const headerDoc = await vscode.workspace.openTextDocument(parsed.headerPath);
    const sourceDoc = await vscode.workspace.openTextDocument(parsed.sourcePath);
    const headerEdit = new vscode.WorkspaceEdit();
    const sourceEdit = new vscode.WorkspaceEdit();
    ...
    if (parsed.xmlPath) {
        await (0, xmlEditor_1.changeXmlElement)(parsed.xmlPath, oldElement, newElement);
    }
    vscode.window.showInformationMessage(`Changed ${oldElement.kind}: ${oldElement.codeName} → ${newElement.codeName}`);
}
function findLinesToRemove(text, element, className, fileType) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        switch (element.kind) {
            ...
            case 'Connector':
                ...
                break;
            case 'State':
                if (fileType === 'header' && line.includes(`Process${codeName}`)) {
                    toRemove.push(i);
                }
                if (fileType === 'source' && line.includes(`RegisterStateProcess`) && line.includes(`"${xmlName}"`)) {
                    toRemove.push(i);
                }
                break;
            case 'StateTransition': {
                ...
                if (fileType === 'header' && line.includes(`Transition${from}To${to}`)) {
                    toRemove.push(i);
                }
                if (fileType === 'source' && line.includes(`RegisterStateTransitionHandler`) && line.includes(`"${from}"`) && line.includes(`"${to}"`)) {
                    toRemove.push(i);
                }
                break;
            }
            case 'Message':
                if (fileType === 'header' && line.includes(`Message${codeName}`)) {
                    toRemove.push(i);
                }
                if (fileType === 'source' && line.includes(`RegisterMessage`) && line.includes(`"${xmlName}"`)) {
                    toRemove.push(i);
                }
                break;
        }
...
function findFunctionBodyLines(sourceText, element, className) {
...
function replaceElementReferences(text, oldEl, newEl, className) {
    let result = text;
