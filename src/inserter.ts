...
function findLinesToRemove(text: string, element: ElementInfo, className: string, fileType: 'header' | 'source'): number[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    switch (element.kind) {
      case 'Signal':
        if (fileType === 'header' && /Signal<.*>\s+/.test(line) && line.includes(codeName)) { toRemove.push(i); }
        if (fileType === 'source' && line.includes(`${codeName}.Create(`)) { toRemove.push(i); }
        break;
      case 'Parameter':
        if (fileType === 'header' && /CDPParameter/.test(line) && line.includes(codeName)) { toRemove.push(i); }
        if (fileType === 'source' && line.includes(`${codeName}.Create(`)) { toRemove.push(i); }
        break;
      case 'Alarm':
        if (fileType === 'header' && /CDPAlarm/.test(line) && line.includes(codeName)) { toRemove.push(i); }
        if (fileType === 'source' && line.includes(`${codeName}.Create(`)) { toRemove.push(i); }
        break;
      case 'Property':
        if (fileType === 'header' && /CDPProperty<.*>/.test(line) && line.includes(codeName)) { toRemove.push(i); }
        if (fileType === 'source' && line.includes(`${codeName}.Create(`)) { toRemove.push(i); }
        break;
      case 'Connector':
        if (fileType === 'header' && /CDPConnector/.test(line) && line.includes(codeName)) { toRemove.push(i); }
        if (fileType === 'source' && line.includes(`${codeName}.Create(`)) { toRemove.push(i); }
        break;
      case 'State':
